...
import { getCityCounts, getTagCounts, getSourceCounts, getGroupCounts } from "@/lib/queries";
...
export default async function SubscribePage() {
  const [cityRows, tagRows, sourceRows, groupRows] = await Promise.all([
    getCityCounts(),
    getTagCounts(),
    getSourceCounts(),
    getGroupCounts(),
  ...
  const groups = groupRows.map((r) => ({ slug: r.slug, name: r.name, count: r.count }));
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-12">
      ...
      <p className="mt-4 max-w-[62ch] text-pretty text-ink-soft leading-relaxed">
        Stack any combination of regions, cities, tags, groups, sources, and
        types below. Then choose how you want it - piped into your calendar, or
        as a weekly Monday morning email. Same filters, your call.
      </p>

      <div className="mt-12">
        <FeedBuilder cities={cities} tags={tags} sources={sources} groups={groups} />
      </div>
